...
/**
 * Simulated MIDI output (syscall 31). The implementation clamps values to the
 * ranges used by the original MARS syscall and forwards the request to
 * handlers if present. In the absence of a dedicated audio device, the syscall
 * simply acknowledges the call.
 */
export function createMidiOutSyscall(devices: SyscallDevices, handlers: HandlerMap): SyscallImplementation {
  return (state): void => {
    ...
    if (!tryHandler("midi_out", handlers, pitch, duration, instrument, volume)) {
      const terminal = devices.terminal;
      terminal?.printString?.(
        `MIDI OUT pitch=${pitch} duration=${duration} instrument=${instrument} volume=${volume}\n`,
      );
    }
